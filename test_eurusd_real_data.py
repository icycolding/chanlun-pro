...
def test_eurusd_with_real_data():
    """
    使用真实数据测试EURUSD新闻搜索功能
    """
    print("EURUSD新闻搜索功能测试 - 使用真实数据")
    print("=" * 60)
    ...
    try:
            ...
            try:
                ...
                if results and len(results) > 0:
                    ...
                    for i, result in enumerate(results, 1):
                        ...
                        eurusd_keywords = ['EURUSD', 'EUR/USD', '欧元', '美元', 'ECB', 'Fed', '欧洲央行', '美联储']
                        found_keywords = [kw for kw in eurusd_keywords if kw.lower() in content.lower()]
                        if found_keywords:
        ...
        if total_found == 0:
            print("⚠️  警告: 没有找到任何EURUSD相关新闻")
            print("   可能原因:")
            print("   1. 数据库中缺少外汇相关新闻")
            print("   2. 新闻内容与EURUSD关联度较低")
            print("   3. 搜索关键词需要优化")
            return False
        else:
            print(f"✓ 成功找到 {total_found} 条EURUSD相关新闻")
    ...
    try:
        ...
        print("✓ ForexCurrencyMapping 模块导入成功")
    ...
    print("\n=== 测试总结 ===")
    print("✅ EURUSD真实数据搜索测试完成")
    print("\n测试要点:")
    print("1. ✓ 使用真实的向量数据库数据")
    print("2. ✓ 测试多种EURUSD输入格式")
    print("3. ✓ 验证搜索结果的相关性")
...
if __name__ == "__main__":
    try:
        ...
        # 然后进行真实数据测试
        success = test_eurusd_with_real_data()
        
        if success:
            print("\n🎉 EURUSD真实数据测试成功完成!")
        else:
            print("\n⚠️  EURUSD真实数据测试遇到问题，请检查数据库内容")
