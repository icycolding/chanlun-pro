...
class TableByCommunityDiscussion(Base):
    __tablename__ = "cl_community_discussion"
    id = Column(Integer, primary_key=True, autoincrement=True, comment="主键ID")
    platform = Column(String(50), nullable=False, comment="平台")
    content_type = Column(String(20), nullable=False, comment="内容类型 post/comment")
    platform_item_id = Column(String(100), nullable=False, comment="平台侧内容ID")
    parent_item_id = Column(String(100), comment="父级内容ID")
    symbol = Column(String(50), comment="标的代码")
    company_name = Column(String(100), comment="公司名称")
    title = Column(String(500), comment="标题")
    content = Column(Text, comment="正文")
    author_name = Column(String(100), comment="作者")
    published_at = Column(DateTime, comment="发布时间")
    like_count = Column(Integer, comment="点赞数")
    reply_count = Column(Integer, comment="回复数")
    url = Column(String(1000), comment="原始链接")
    payload_json = Column(Text, comment="原始载荷")
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "content_type",
            "platform_item_id",
            name="uq_community_discussion_platform_type_item",
        ),
        {"mysql_collate": "utf8mb4_general_ci"},
    )


class TableByCommunityDiscussionAssetLink(Base):
    __tablename__ = "cl_community_discussion_asset_link"
    id = Column(Integer, primary_key=True, autoincrement=True, comment="主键ID")
    discussion_id = Column(Integer, nullable=False, index=True, comment="社区讨论ID")
    canonical_asset = Column(String(50), nullable=False, index=True, comment="标准资产代码")
    relation_type = Column(String(20), comment="关系类型 direct/background")
    confidence = Column(Float, comment="置信度", default=0.5)
    reason = Column(String(200), comment="关联原因")
    created_at = Column(DateTime, nullable=False, default=datetime.datetime.now)
    updated_at = Column(DateTime, nullable=False, default=datetime.datetime.now, onupdate=datetime.datetime.now)
    __table_args__ = (
        UniqueConstraint(
            ...
            "canonical_asset",
            "relation_type",
            name="uq_community_discussion_asset_link",
        ),
        {"mysql_collate": "utf8mb4_general_ci"},
    )
...
class DB(object):
    ...
    def community_discussions_replace_or_upsert(self, rows: List[dict]) -> bool:
        with self.Session() as session:
            ...
            for row in rows or []:
                ...
                content_type = str(row.get("content_type") or "post").strip().lower()
                platform_item_id = str(row.get("platform_item_id") or "").strip()
                if not platform or not platform_item_id:
                    continue

                existing = (
                    session.query(TableByCommunityDiscussion)
                    .filter(
                        TableByCommunityDiscussion.platform == platform,
                        TableByCommunityDiscussion.content_type == content_type,
                        TableByCommunityDiscussion.platform_item_id == platform_item_id,
                    )
                    .first()
                )
                payload = {
                    ...
                    "url": row.get("url"),
                    "payload_json": json.dumps(row.get("raw_payload") or {}, ensure_ascii=False),
                    "updated_at": row.get("updated_at") or now,
                }
                if existing:
                    for key, value in payload.items():
                    ...
                    session.add(TableByCommunityDiscussion(**payload))
            session.commit()
        return True

    def community_discussions_query(
        self,
        *,
        symbol: str = None,
        ...
        with self.Session() as session:
            query = session.query(TableByCommunityDiscussion)
            if symbol:
                query = query.filter(TableByCommunityDiscussion.symbol == symbol)
            if company_name:
                like_term = f"%{company_name}%"
                query = query.filter(TableByCommunityDiscussion.company_name.like(like_term))
            if platform:
                query = query.filter(TableByCommunityDiscussion.platform == platform.strip().lower())
            if content_type:
                query = query.filter(TableByCommunityDiscussion.content_type == content_type.strip().lower())
            rows = (
                query.order_by(
                    TableByCommunityDiscussion.published_at.desc().nullslast(),
                    TableByCommunityDiscussion.updated_at.desc(),
                )
                .limit(limit)
                .all()
            )
            return [
                {
                    ...
                    "platform_item_id": row.platform_item_id or "",
                    ...
                    "published_at": fun.datetime_to_str(row.published_at) if row.published_at else "",
    ...
    def community_discussion_asset_links_replace(self, rows: List[dict]) -> bool:
        with self.Session() as session:
            now = datetime.datetime.now()
            for row in rows or []:
                discussion_id = row.get("discussion_id")
                canonical_asset = str(row.get("canonical_asset") or "").strip()
                relation_type = str(row.get("relation_type") or "direct").strip().lower()
                if not discussion_id or not canonical_asset:
                    continue
                existing = (
                    session.query(TableByCommunityDiscussionAssetLink)
                    .filter(
                        TableByCommunityDiscussionAssetLink.discussion_id == discussion_id,
                        TableByCommunityDiscussionAssetLink.canonical_asset == canonical_asset,
                        TableByCommunityDiscussionAssetLink.relation_type == relation_type,
                    )
                    .first()
                )
                payload = {
                ...
                if existing:
                    ...
                    session.add(TableByCommunityDiscussionAssetLink(**payload))
            session.commit()
        return True
